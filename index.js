...
app.post('/register', ifLoggedin,
    ...
    [
        ...
        body('pwd', 'The password must be of minimum length 6 characters').trim().isLength({ min: 6 }),
    ...
    (req, res, next) => {
        ...
        if (validation_result.isEmpty()) {
            ...
            bcrypt.hash(pwd, 12).then((hash_pass) => {
...
app.post('/', ifLoggedin, [
    ...
    if (validation_result.isEmpty()) {
        ...
        dbConnection.execute("SELECT * FROM `users` WHERE `email`=?", [email])
            .then(([rows]) => {
                bcrypt.compare(pwd, rows[0].password).then(compare_result => {
